window.addEventListener('load', function() {
    ...
    function initAudioAndStartGame() {
        ...
        audioContext.resume().then(() => {
            console.log("AudioContext jest gotowy i działa.");
            allAssetsLoaded = true; // Ustaw flagę, że można już odtwarzać dźwięki
            startGame();
        }).catch(e => {
            console.error("Nie udało się wznowić AudioContext:", e);
            // Nawet jeśli audio zawiedzie, uruchom grę bez dźwięku
            startGame();
        });
    }
    
    // Uruchamia faktyczną grę
    function startGame() {
    ...
    // Ładuje wszystkie zasoby w tle po załadowaniu strony
    async function loadInitialAssets() {
        try {
            ...
            // Tworzymy kontekst w tle, będzie "uśpiony"
            if (!audioContext) {
                audioContext = new (window.AudioContext || window.webkitAudioContext)();
            }
            ...
            console.log("Wszystkie zasoby załadowane w tle.");
            // Gdy wszystko gotowe, zmień tekst na ekranie startowym
            ...
            console.error("Błąd podczas ładowania zasobów w tle:", error);
