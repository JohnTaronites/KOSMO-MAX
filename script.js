window.addEventListener('load', function() {
    ...
    function initAndStartGame() {
        // Natychmiast usuń listenery, aby zapobiec ponownemu wywołaniu
        window.removeEventListener('click', initAndStartGame);
        window.removeEventListener('touchstart', initAndStartGame);

        // Pokaż graczowi, że coś się dzieje
        startScreen.innerHTML = '<h1>ŁADOWANIE...</h1>';
        startScreen.style.cursor = 'default';

        // 1. Stwórz AudioContext w odpowiedzi na akcję użytkownika
        if (!audioContext) {
            audioContext = new (window.AudioContext || window.webkitAudioContext)();
        }

        // 2. NATYCHMIAST spróbuj wznowić kontekst.
        audioContext.resume().then(() => {
            // Ten kod wykona się, gdy przeglądarka "obudzi" silnik audio
            console.log("AudioContext gotowy i działa. Stan:", audioContext.state);
            
            // 3. Dopiero TERAZ, gdy audio jest aktywne, ładuj wszystkie zasoby
            loadAllAssets();

        }).catch(error => {
            console.error("Błąd krytyczny przy wznawianiu AudioContext:", error);
            startScreen.innerHTML = '<h1>Błąd audio</h1><p>Twoja przeglądarka zablokowała dźwięk. Spróbuj odświeżyć stronę lub sprawdź jej ustawienia.</p>';
        });
    }
