window.addEventListener('load', function() {
    ...
    // --- ZMIANA: Modyfikacja UI, aby uwzględnić wskaźnik poziomu ---
    ...
    uiContainer.style.width = '100%'; // Zajmuje całą szerokość
    uiContainer.style.display = 'flex'; // Używamy flexboxa do rozmieszczenia
    uiContainer.style.justifyContent = 'space-between'; // Rozkłada elementy równomiernie
    uiContainer.style.padding = '10px 20px'; // Lepszy padding
    ...
    const levelEl = document.createElement('div'); // NOWY element
    const livesEl = document.createElement('div');
    
    uiContainer.appendChild(scoreEl);
    uiContainer.appendChild(levelEl); // Dodajemy go do UI
    ...
    // --- ZMIANA: Przebudowana, bezpieczniejsza funkcja ---
    function handleGameElements(deltaTime) {
        ...
        for (let i = bullets.length - 1; i >= 0; i--) {
            for (let j = enemies.length - 1; j >= 0; j--) {
                // Upewniamy się, że oba obiekty na pewno istnieją
        ...
        // 4. Czystka - usuwanie oznaczonych obiektów
        bullets = bullets.filter(bullet => !bullet.markedForDeletion);
        enemies = enemies.filter(enemy => !enemy.markedForDeletion);
        
        // 5. Rysowanie wszystkiego, co zostało
    ...
    // ZMIANA: updateUI aktualizuje teraz także poziom
    function updateUI() {
    ...
    function showLevelUpMessage(level) {
        ...
        levelUpEl.style.position = 'absolute'; levelUpEl.style.left = '50%'; levelUpEl.style.top = '50%'; levelUpEl.style.transform = 'translate(-50%, -50%)'; levelUpEl.style.color = '#6c6cff'; levelUpEl.style.fontSize = '5em'; levelUpEl.style.textShadow = '3px 3px 6px #000'; levelUpEl.style.opacity = '1'; levelUpEl.style.transition = 'opacity 1s ease-out';
        gameContainer.appendChild(levelUpEl);
        setTimeout(() => { levelUpEl.style.opacity = '0'; setTimeout(() => { gameContainer.removeChild(levelUpEl); }, 1000); }, 1500);
    }
    function levelUp(newLevel) {
        ...
        if (currentLevel === 3) { maxSuperShotCharges = 3; }
        superShotCharges = maxSuperShotCharges;
    }
    function checkLevelUp() {
        if (currentLevel === 1 && score >= 500) { levelUp(2); }
        else if (currentLevel === 2 && score >= 2000) { levelUp(3); }
    }
    ...
    function animate(timestamp) {
        ...
        if (gameOver) { if (gameOverScreen.style.display !== 'flex') { playSound('gameOver'); setTimeout(() => { gameOverScreen.style.display = 'flex'; finalScoreEl.innerText = score; }, 500); } }
